...
{
    public partial class MachMachines : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            bool authorizated = masterPage.DefineUserRights(role, "MachMachines");
            if (authorizated)
            {
                if (!IsPostBack)
                {                 
                    ...
                    MachIDResults.DataSource = dataTableStatus;
                    MachIDResults.DataBind();

                    int Backupchecked,Configchecked = 0;

                    CheckBox chkRow;

                    foreach (GridViewRow row in MachIDResults.Rows) //Running all lines of grid
                    {
                        for (int i = 4; i < 14; i++)
                        {
                    ...
                    SelectionCell.DataSource = sQLQuery.GetCellName("MACHINING");
        ...
        protected void btnSaveStatus_ServerClick(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in MachIDResults.Rows)
            {
